...
if "BncrHost" in os.environ:
    if len(os.environ["BncrHost"]) > 1:
        BncrHost = os.environ["BncrHost"]

if "BncrToken" in os.environ:
    if len(os.environ["BncrToken"]) > 1:
        BncrToken = os.environ["BncrToken"]

if BncrHost and BncrToken:
    notify_mode.append('Bncr')
if BARK:
...
def bncr(title, content):
    ...
    response = requests.post(f"{BncrHost}/api/qinglongMessage", data=data).json()
...
def send(title, content):
    ...
    for i in notify_mode:
        if i == 'Bncr':
            if BncrHost and BncrToken:
                bncr(title=title, content=content)
            else:
                print('未启用 Bncr')
            continue
        if i == 'bark':
